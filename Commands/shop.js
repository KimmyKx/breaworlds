...
module.exports = new Command({
	...
	async run(message, args, client, user) {
		...
		const result = shop.map((item) => `**x${item.count.toLocaleString()} ${item.id.getFieldById(item.category)?.name || item.name || ''}** — ${item.price[0].getLogo()} ${item.price[1].toLocaleString()} \n${item.description} \nID: \`${item.id}\`\n`);
		let msg;
		start(1);
